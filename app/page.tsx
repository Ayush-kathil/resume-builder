...
import { ScanSearch, Sparkles, Layout } from 'lucide-react';
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#F2F1ED] text-[#1a1a1a] font-sans selection:bg-[#1a1a1a] selection:text-[#F2F1ED] pb-20">
      {/* Navigation — responsive: hides nav items behind a simple flex layout; no hamburger needed at this minimal scale */}
      <nav className="w-full flex items-center justify-between p-6 md:px-12 lg:px-24">
        ...
        <div className="flex items-center gap-4 md:gap-6 text-sm font-medium">
          ...
          {/* Fix UI: Link→button anti-pattern removed. Style applied directly to Link. */}
          <Link
            href="/signup"
            className="bg-[#1a1a1a] text-[#F2F1ED] px-4 md:px-5 py-2.5 rounded-full hover:bg-black transition-colors"
          >
            Sign up
          </Link>
        </div>
      ...
      {/* Hero Section */}
      <section className="flex flex-col items-center pt-16 pb-12 px-6 md:pt-32 md:pb-24 text-center">
        <motion.div
        ...
        >
          <h1 className="text-5xl md:text-7xl lg:text-8xl font-playfair font-medium tracking-tight leading-[1.05] mb-8 break-words hyphens-auto">
            A perfect resume for every application
          </h1>
          
          {/* Fix UI: button inside Link replaced with styled Link */}
          <Link
            href="/signup"
            className="group flex items-center gap-3 bg-[#1a1a1a] text-[#F2F1ED] px-6 py-3 rounded-full font-medium hover:bg-black hover:scale-[1.02] active:scale-[0.98] transition-all duration-200"
          >
      ...
      {/* Marquee Section — Fix: single-container seamless marquee using duplicated inline items */}
      <section className="w-full py-24 overflow-hidden bg-[#F2F1ED]">
        ...
        <div className="relative flex overflow-x-hidden">
          {/* Fix: hover-pause via group-hover:[animation-play-state:paused] */}
          <div className="flex animate-marquee group-hover:[animation-play-state:paused] whitespace-nowrap items-center gap-16 md:gap-24 px-8">
            {['Google','Meta','Apple','Netflix','Amazon','Microsoft','Stripe','Airbnb','Google','Meta','Apple','Netflix','Amazon','Microsoft','Stripe','Airbnb'].map((name, i) => (
              <MarqueeItem key={i} text={name} />
            ))}
          </div>
        </div>
      </section>

      {/* Features Grid — Fix: meaningful icons per card */}
      <section className="py-24 px-6 md:px-12 max-w-7xl mx-auto border-t border-[#e5e5e5]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <FeatureCard 
            ...
            title="ATS Optimized"
            description="Tested against top Applicant Tracking Systems to ensure perfect parsing and keyword matching."
          />
          <FeatureCard 
          ...
          <FeatureCard 
            ...
            description="Export to pixel-perfect PDF layouts designed by top-tier typographers. Print-ready every time."
          />
        </div>
      </section>
      ...
      <section className="py-32 px-6 md:px-12 bg-white text-center">
        <div className="max-w-4xl mx-auto">
          <h3 className="text-3xl md:text-5xl font-playfair font-medium italic leading-relaxed mb-8">
            &ldquo;I spent hours formatting my old CV. With Resume Maker, I had a gorgeous, perfectly tailored resume in 5 minutes that landed me my dream role.&rdquo;
          </h3>
          <p className="font-medium text-gray-500 uppercase tracking-widest text-sm">— Senior Engineer, Meta</p>
        </div>
      </section>

      {/* Final CTA — Fix: button inside Link replaced with styled Link */}
      <section className="py-32 px-6 text-center bg-[#F2F1ED]">
        ...
        <Link
          href="/signup"
          className="group inline-flex items-center gap-3 bg-[#1a1a1a] text-[#F2F1ED] px-8 py-4 rounded-full font-medium hover:bg-black hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 mx-auto text-lg"
        >
...
function FeatureCard({ title, description, icon }: { title: string; description: string; icon: React.ReactNode }) {
  return (
    <div className="p-8 rounded-[2rem] bg-white border border-[#e5e5e5] hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
      <div className="w-12 h-12 bg-[#F2F1ED] rounded-full mb-6 flex items-center justify-center text-[#1a1a1a]">
        {icon}
      </div>
